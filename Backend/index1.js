...
app.post("/bookingdetails", async (req, res) => {
  ...
  try {
    ...
    // Send the details data as a response
...
app.post("/cabbook", async (req, res) => {
  const Guestid = req.body.Guestid;

  try {
    // Check if the user already exists
    const details = await db.query(
      "SELECT * FROM cab_services where guest_id = $1 ",
      [Guestid]
    );
...
app.post("/foodbook", async (req, res) => {
  const Guestid = req.body.Guestid;

  try {
    // Check if the user already exists
    const details = await db.query(
      "SELECT * FROM food_ordered natural join reservations natural join food where guest_id = $1 ",
      [Guestid]
    );
...
app.post("/getuser", async (req, res) => {
  ...
  try {
    // Check if the user already exists
    const details = await db.query(
      "SELECT name FROM guests where guest_id = $1 ",
      [Guestid]
    );
...
app.post("/addreview", async (req, res) => {
  const { guest_id, review, rating } = req.body;

  try {
    // Execute the INSERT statement
    await db.query(
      `INSERT INTO reviews (guest_id, reviews, ratings) VALUES ($1, $2, $3)`,
      [guest_id, review, rating]
    );
